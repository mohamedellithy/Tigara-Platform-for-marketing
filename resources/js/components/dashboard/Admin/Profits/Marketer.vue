                            ...
                            <ul class="filter-results">
                                ...
                                <li class="filter-item">
                                    <i class="fas fa-user-slash"></i>
                                     {{ all_marketer_and_platform_orders_profits }}  اجمالى الأرباح
                                </li>
                                <li class="filter-item">
                                    <i class="fas fa-user-check"></i>
                                    {{ total_profites }}  اجمالى الأرباح المسوقين
                                </li>
                    ...
                    <nav v-if="this.infos.length != 0" aria-label="Page navigation example">
                        <ul v-if="this.infos.total > marketers.length" class="pagination">
                            <li v-if="(this.infos.current_page != 1)" class="page-item">
                                <router-link class="page-link" :to="{path: '/dashboard/marketer-profites/'+(this.infos.current_page - 1 == 0 ? 1 : this.infos.current_page - 1) }" aria-label="Previous">
                            ...
                            <li v-for="page in this.infos.last_page" class="page-item" :key="page">
                                <template v-if="page == 1">
                                    <router-link class="page-link" :to="{path: '/dashboard/marketer-profites/'+page}" active-class="active" exact>{{ page }}</router-link>
                                </template>
                                ...
                                <template v-else-if="(page == this.infos.current_page - 2) && (this.infos.current_page != this.infos.last_page)">
                                    <router-link class="page-link" :to="{path: '/dashboard/marketer-profites/'+page}" active-class="active" exact>..</router-link>
                                </template>
                                <template v-else-if="(page == this.infos.current_page + 2) && (this.infos.current_page != this.infos.last_page)">
                                    <router-link class="page-link" :to="{path: '/dashboard/marketer-profites/'+page}" active-class="active" exact>..</router-link>
                                </template>
                            ...
                            <li v-if="this.infos.current_page != this.infos.last_page" class="page-item">
                                <router-link :to="{path: '/dashboard/marketer-profites/'+(this.infos.current_page + 1) }" class="page-link" href="#" aria-label="Next">
...
export default {
    data() {
        return {
            total_profites:0,
            all_marketer_and_platform_orders_profits:0,
    ...
    methods:{
        FetchMarketers:function(){
            ...
            axios.get('/api/marketers',{
                ...
                self.all_marketer_and_platform_orders_profits = data.all_marketer_and_platform_orders_profits;
                self.total_profites = data.total_profites;
