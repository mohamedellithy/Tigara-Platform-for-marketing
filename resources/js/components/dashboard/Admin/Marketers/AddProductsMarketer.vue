             ...
             <div class="marketer-detailes">
                <ul class="banner-profile-marketer">
                    ...
                    <li class="label-count-products">
                        المنتجات المحددة للمسوق ( {{ marketer.products.length }} ) منتجات
                    </li>
                    <li class="label-name-marketer"> 
                    ...
                    <li class="label-marketer-details">
                        <router-link :to="{path:'/dashboard/show-products-marketer/'+this.$route.params.id}" class="btn btn-primary btn-sm" style="margin:0px 10px">عرض منتجات المسوق</router-link>
                    </li>
                    <li class="label-marketer-details">
            ...
            <nav v-if="this.infos.length != 0" aria-label="Page navigation example">
                <ul v-if="this.infos.total > products.length" class="pagination">
                    <li v-if="(this.infos.current_page != 1)" class="page-item">
                        <router-link class="page-link" :to="{path: '/dashboard/add-products-marketer/'+this.$route.params.id+'/'+(this.infos.current_page - 1 == 0 ? 1 : this.infos.current_page - 1) }" aria-label="Previous">
                    ...
                    <li v-for="page in this.infos.last_page" class="page-item" :key="page">
                        <template v-if="page == 1">
                            <router-link class="page-link" :to="{path: '/dashboard/add-products-marketer/'+this.$route.params.id+'/'+page}" active-class="active" exact>{{ page }}</router-link>
                        </template>
                        ...
                        <template v-else-if="(page == this.infos.current_page - 2) && (this.infos.current_page != this.infos.last_page)">
                            <router-link class="page-link" :to="{path: '/dashboard/add-products-marketer/'+this.$route.params.id+'/'+page}" active-class="active" exact>..</router-link>
                        </template>
                        <template v-else-if="(page == this.infos.current_page + 2) && (this.infos.current_page != this.infos.last_page)">
                            <router-link class="page-link" :to="{path: '/dashboard/add-products-marketer/'+this.$route.params.id+'/'+page}" active-class="active" exact>..</router-link>
                        </template>
                    ...
                    <li v-if="this.infos.current_page != this.infos.last_page" class="page-item">
                        <router-link :to="{path: '/dashboard/add-products-marketer/'+this.$route.params.id+'/'+(this.infos.current_page + 1) }" class="page-link" href="#" aria-label="Next">
...
export default {
    ...
    methods:{
        FetchProducts:function(){
            let self = this;
            axios.get('/api/marketer-nothave-products/'+self.$route.params.id,{
                params:self.params
            }).then(function({data}){
                console.log(data);
        ...
        AddProductsToMarketer:function(product_id){
            let self = this;
            self.field.products.push(product_id);
            axios.post('/api/add-products-to-marketer/' + self.$route.params.id,self.field).then(function({data}){
                console.log(data);
                self.field.products = [];
